...
export default function Index({ durations, success }) {
 ...
 const [initialRecords, setInitialRecords] = useState(sortBy(durations, 'id'));
 ...
 const cols = [
   ...
   { accessor: 'type_duration', title: 'Type' },
   ...
   setInitialRecords(() => {
     return durations.filter((item) => {
       return (
         ...
         item.type_duration.toLowerCase().includes(search.toLowerCase()) ||
 ...
 //FIN DATA TABLE
  ...
  const handleClick = (id) => {
    Inertia.get(route('dashboard.duration-convention.edit', id));
  }

  const handelState=(modalId)=>{
    Inertia.get(route('dashboard.duration-convention.show', modalId));
  }
    ...
    <DefaultLayout>
      <div className="panel datatables">
        <div className="flex justify-between mb-5">
          <h3 className="text-lg">Durée d'une convention</h3>
         
        </div>

        <div className="flex md:items-center md:flex-row flex-col mb-5 gap-5">
        <Link href={route('dashboard.duration-convention.create')} className="btn btn-success">Création</Link>
        ...
        <DataTable
          ...
          columns={[
            ...
            {
              accessor: 'type_duration',
              title: 'Type',
              sortable: true,
              hidden: hideCols.includes('type_duration'),
              render: (row) => (
                toPascalCase(row.type_duration)
              )
            },
            {
              accessor: 'status',
