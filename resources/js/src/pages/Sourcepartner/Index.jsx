...
const [initialRecords, setInitialRecords] = useState(sortBy(sources, 'id'));
...
const cols = [
  ...
  { accessor: 'type_source', title: 'Type' },
  ...
  setInitialRecords(() => {
    return sources.filter((item) => {
      return (
        ...
        item.type_source.toLowerCase().includes(search.toLowerCase()) ||
...
//FIN DATA TABLE
  ...
  const [modalId, setModalId] = useState("");
   ...
   const handleClick=(id)=>{
    Inertia.get(route('dashboard.liste-source.edit',id));
  }
  ...
  const handelState=(modalId)=>{
    Inertia.get(route('dashboard.liste-source.show',modalId));
  }
  const boutons = [
    ...
    <DefaultLayout>
      <div className="panel datatables">
        <div className="flex justify-between mb-5">
          <h3 className="text-lg">Liste des sources</h3>
         
        </div>
        
        <div className="flex md:items-center md:flex-row flex-col mb-5 gap-5">
        <Link href={route('dashboard.liste-source.create')} className="btn btn-success">Création</Link>
        ...
        <DataTable
          ...
          columns={[
            { accessor: 'id', title: 'Id', sortable: true, hidden: hideCols.includes('id') },
            {
              ...
              title: 'Type',
              sortable: true,
              hidden: hideCols.includes('type_source'),
              render: (row) => (
                toPascalCase(row.type_source)
              )
            },
            {
              accessor: 'status',
