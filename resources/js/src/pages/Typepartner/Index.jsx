...
const cols = [
  ...
  { accessor: 'type_partner', title: 'Type' },
  ...
  setInitialRecords(() => {
    return partenaires.filter((item) => {
      return (
        ...
        item.type_partner.toLowerCase().includes(search.toLowerCase()) ||
...
//FIN DATA TABLE
  ...
  useEffect(()=>{
   ...
   const handleClick=(id)=>{
    Inertia.get(route('dashboard.liste-partenaire-type.edit',id));
  }
  ...
  const handelState=(modalId)=>{
    Inertia.get(route('dashboard.liste-partenaire-type.show',modalId));
  }
  const boutons = [
    ...
    <DefaultLayout>
      <div className="panel datatables">
        <div className="flex justify-between mb-5">
          <h3 className="text-lg ">Liste de type de partenaire</h3>
        </div>
        <div className="flex md:items-center md:flex-row flex-col mb-5 gap-5">
        <Link href={route('dashboard.liste-partenaire-type.create')} className="btn btn-success">Création</Link>
        ...
        <DataTable
          ...
          columns={[
            ...
            {
              accessor: 'type_partner',
              title: 'Type',
              sortable: true,
              hidden: hideCols.includes('type_partner'),
              render: (row) => (
                toPascalCase(row.type_partner)
              )
            },
            {
              accessor: 'status',
