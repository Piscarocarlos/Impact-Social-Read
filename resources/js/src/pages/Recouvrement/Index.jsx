...
import { FiX, FiEdit, FiZap,FiEye } from "react-icons/fi";
import { Inertia } from '@inertiajs/inertia';
import { Dialog, Transition } from '@headlessui/react';
import Swal from 'sweetalert2';


export default function Index({ recouvrement, success}) {
   ...
   const [initialRecords, setInitialRecords] = useState(sortBy(recouvrement, 'id'));
   ...
   const cols = [
     ...
     { accessor: 'partenaire', title: 'Partenaire' },
   ];
  ...
  const handelState=(modalId)=>{
    Inertia.get(route('dashboard.recouvrement.edit',modalId));
  }
  ...
  const dataRecouvre=(row)=>{
    return JSON.parse(row)
  }

console.log('data',JSON.parse(recouvrement[0].data).partnerId['label'])
    ...
    <DefaultLayout>
      <div className="panel datatables">
        <div className="flex justify-between mb-5">
          <h3 className="text-lg">Recouvrement</h3>
        </div>
        ...
        <DataTable
          ...
          columns={[
            ...
            { accessor: 'id', title: 'Partenaire', sortable: true, hidden: hideCols.includes('id'),render:(row)=>(
              JSON.parse(row.data).partnerId['label']
            ) },
