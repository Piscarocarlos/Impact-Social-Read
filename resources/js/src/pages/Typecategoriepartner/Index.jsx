...
export default function Index({ categories,typePartenaire,success }) {
   ...
   const [initialRecords, setInitialRecords] = useState(sortBy(categories, 'id'));
   ...
   const cols = [
     ...
     { accessor: 'name_categorie', title: 'Catégorie' },
     { accessor: 'type_partner_id', title: 'Type partenaire' },
     ...
     setInitialRecords(() => {
       return categories.filter((item) => {
         return (
           ...
           item.name_categorie.toLowerCase().includes(search.toLowerCase()) ||
           item.status.toString().includes(search.toLowerCase()) ||
           item.type_partner_id.toString().includes(search.toLowerCase())
         );
       });
     });
   }, [search]);
  ...
  const findUserById = (id) => {
    const user = typePartenaire.find(item => item.id === id);
    return user ? user.type_partner : null;
  };
  ...
  useEffect(()=>{
   ...
   const handleClick=(id)=>{
    Inertia.get(route('dashboard.liste-categorie-partenaire.edit',id));
  }
  ...
  const handelState=(modalId)=>{
    Inertia.get(route('dashboard.liste-categorie-partenaire.show',modalId));
  }
  const boutons = [
    ...
    <DefaultLayout>
      <div className="panel datatables">
        <div className="flex justify-between mb-5">
          <h3 className="text-lg">Catégorie des partenaires</h3>
        </div>
        
        <div className="flex md:items-center md:flex-row flex-col mb-5 gap-5">
        <Link href={route('dashboard.liste-categorie-partenaire.create')} className="btn btn-success">Création</Link>
        ...
        <DataTable
          ...
          columns={[
            ...
            {
              accessor: 'name_categorie',
              title: 'Catégorie',
              sortable: true,
              hidden: hideCols.includes('name_categorie'),
              render: (row) => (
                toPascalCase(row.name_categorie)
              )
            },
            {
              accessor: 'type_partner_id',
              title: 'Type partenaire',
              sortable: true,
              hidden: hideCols.includes('type_partner_id'),
              render: (row) => (
                toPascalCase(findUserById(row.type_partner_id))
              )
            },
            {
              accessor: 'status',
