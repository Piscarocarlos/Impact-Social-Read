...
export default function Create(){
    ...
    const { type_partenaires,categorie } = usePage().props;

    const { data, put, setData, processing, errors } = useForm({
        name_categorie: categorie.name_categorie,
        type_partner:categorie.id
    })

    const submit = (e) => {
        e.preventDefault();
        put(route('dashboard.liste-categorie-partenaire.update',categorie.id), {
    ...
    <DefaultLayout>
      <div >
        <div className="flex items-center justify-between mb-5">
          <h5 className="font-semibold text-lg dark:text-white-light">Modification catégorie du partenaire</h5>
        </div>
        <div className="datatables">
          <div className=" flex justify-between flex-nowrap dark:text-white gap-4">
            <div className="md:w-3/5 sm:w-full">
              <div className="panel">
                <h2 className="text-lg mb-3 font-bold">Modifier </h2>
