...
export default function Index({ convention}) {
  ...
  const cols = [
    { accessor: 'id', title: 'Convention' },
    { accessor: 'familleConvention', title: 'Famille de convention' },
    { accessor: 'accountBank', title: 'Compte bancaire' },
  ...
  const [modalId, setModalId] = useState("");
   ...
   const handleClick=(id)=>{
    Inertia.get(route('dashboard.convention.edit',id));
  }
  ...
  const handelState=(modalId)=>{
    Inertia.get(route('dashboard.convention.show',modalId));
  }
  const boutons = [
    ...
    <DefaultLayout>
      <div className="panel datatables">
        <div className="flex justify-between mb-5">
          <h3 className="text-lg">Liste des conventions finalisées</h3>
        </div>
        ...
        <DataTable
          ...
          records={recordsData}
          columns={[
            { accessor: 'id', title: 'Convention', sortable: true, hidden: hideCols.includes('id'),
          render:(row)=>(
            ...
            },
            {
              ...
              sortable: true,
              hidden: hideCols.includes('accountBank'),
              render: (row) => (
                handelDataConvention(row.data).accountBank.value
              )
            },
            {
              accessor: 'status',
              ...
              render: (row) => (
                <span className="badge bg-success">{handelDataConvention(row.data).statutConvention.value}</span>
              )
            },
            { accessor: 'created_at', title: 'Date création', sortable: true, hidden: hideCols.includes('created_at') },
            {
              ...
              render: (row) => (
                ...
                <button className="btn btn-primary"  onClick={() =>handelEdit(row.id)}>
                  <FiEye /> 
                </button>
            </div>
                ...
                <div className="flex min-h-screen items-start justify-center px-4">
                  <Dialog.Panel className="panel animate__animated animate__slideInDown my-8 w-full max-w-lg overflow-hidden rounded-lg border-0 p-0 text-black dark:text-white-dark">
                    <div className="flex items-center justify-between bg-[#fbfbfb] px-5 py-3 dark:bg-[#121c2c]">
                      <h5 className="text-lg font-bold">Visualisation</h5>
                    ...
                    <div className="p-5">
                      <p>
                        Voulez-vous ouvrir cet élément ?
                      </p>
