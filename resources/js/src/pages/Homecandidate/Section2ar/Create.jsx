...
export default function Create(){
  ...
  const submit = (e) => {
    e.preventDefault();
    post(route('dashboard.section-two-candidate-ar.store'), {
            ...
            <div className=" sm:w-full">
              <div className="panel">
                ...
                <div className="grid grid-cols-1 gap-3">
                        <div className="my-2">
                            <label htmlFor="section_two_id" className="text-base">Ligne de correspondance en français <span className='text-danger'>*</span></label>
                            <select type="text" value={data.section_two_id} onChange={e => setData('section_two_id', e.target.value)} className="form-select"  >
                                <option defaultValue={'selected'}>Sélectionner une ligne</option>
                                {section.map(item => (
                                <option key={item.id} value={item.id}>{item.titre}</option>
                                ))}
                            </select>
                            {errors.section_two_id && <div className="text-danger">{errors.section_two_id}</div>}
                        </div>
                      </div>
                    <div className="grid grid-cols-1 gap-3">
                        <div className="my-2">
