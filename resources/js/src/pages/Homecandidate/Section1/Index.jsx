...
export default function Index({ section }) {
  ...
  const handleClick=(id)=>{
    Inertia.get(route('dashboard.section-one-candidate.edit',{id}));
  }
  const handelEdit = (id)=>{
    Inertia.get(route('dashboard.section-one-candidate.show',id));
  }
  const handelDelete = (id)=>{
    Inertia.delete(route('dashboard.section-one-candidate.destroy',id));
    ...
    <DefaultLayout>
      <div className="panel">
        <div className="flex justify-between">
          <h3 className="text-xl font-bold">Liste des messages</h3>
          <Link href={route('dashboard.section-one-candidate.create')} className="btn btn-success">Créer un message</Link>
        </div>
