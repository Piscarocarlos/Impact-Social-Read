...
export default function Index({ section }) {
  ...
  const handleClick=(id)=>{
    Inertia.get(route('dashboard.section-two-candidate.edit',{id}));
  }
  const handelEdit = (id)=>{
    Inertia.get(route('dashboard.section-two-candidate.show',id));
  }
  const handelDelete = (id)=>{
    Inertia.delete(route('dashboard.section-two-candidate.destroy',id));
    ...
    <DefaultLayout>
      <div className="panel">
        <div className="flex justify-between">
          <h3 className="text-xl font-bold">Liste des descriptions de la fondation</h3>
          <Link href={route('dashboard.section-two-candidate.create')} className="btn btn-success">Créer une description</Link>
        </div>
