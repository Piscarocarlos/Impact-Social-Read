...
export default function Index({ section }) {
  ...
  const handleClick=(id)=>{
    Inertia.get(route('dashboard.section-three-candidate-ar.edit',{id}));
  }
  const handelEdit = (id)=>{
    Inertia.get(route('dashboard.section-three-candidate-ar.show',id));
  }
  const handelDelete = (id)=>{
    Inertia.delete(route('dashboard.section-three-candidate-ar.destroy',id));
    ...
    <DefaultLayout>
      <div className="panel">
        <div className="flex justify-between">
          ...
          <Link href={route('dashboard.section-three-candidate-ar.create')} className="btn btn-success">Créer un card</Link>
        </div>
        <DataTable
          columns={[
