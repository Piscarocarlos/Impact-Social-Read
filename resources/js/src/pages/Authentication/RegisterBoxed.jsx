...
const RegisterBoxed = () => {
    ...
    const {data, post, get, setData, errors, processing} = useForm({
        name: "",
        email: "",
        password: "",
    });
    ...
    {
        if (password_ref.current.type == "password") {
            password_ref.current.setAttribute("type", "text")
        } else {
            password_ref.current.setAttribute("type", "password")
        }
    }
        ...
        <div>
            ...
            <div className="flex justify-center items-center min-h-screen bg-cover bg-center bg-[url('/assets/images/map.svg')] dark:bg-[url('/assets/images/map-dark.svg')]">
                <div className="panel sm:w-[500px] m-6 max-w-lg w-full">
                    <h2 className="font-bold text-2xl mb-3">Rejoignez-nous </h2>
                    <p className="mb-7">En plus c'est facile, créez maintenant votre compte !</p>
                    <form className="space-y-5" onSubmit={submitForm}>
                        <div>
                            <label htmlFor="name">Votre nom complet <span className="text-danger">*</span></label>
                            <input id="name" value={data.name} onChange={e => setData('name', e.target.value)} type="text" className="form-input" placeholder="Entrez le nom complet sur votre pièce d'identité" />
                            {errors.name && <div className="text-danger">{errors.name}</div>}
                        </div>
                        <div>
                            <label htmlFor="email">Adresse e-mail <span className="text-danger">*</span></label>
                            <input id="email" type="email" value={data.email} onChange={e => setData('email', e.target.value)} className="form-input" placeholder="Entrer votre adresse e-mail valide" />
                        </div>
                        <div>
                            <label htmlFor="password">Mot de passe <span className="text-danger">*</span></label>
                            <div className="flex relative">
                                <input id="iconRight" ref={password_ref} type="password" value={data.password} 
                                 placeholder="****** (Min: 6 caractères)" className="form-input ltr:rounded-r-none rtl:rounded-l-none" onChange={e => setData('password', e.target.value)} />
                                <div onClick={showPassword} className="bg-[#eee] absolute cursor-pointer right-0 top-0 bottom-0 rounded flex justify-center items-center ltr:rounded-r-md rtl:rounded-l-md px-3 font-semibold border ltr:border-l-0 rtl:border-r-0 border-[#e0e6ed] dark:border-[#17263c] dark:bg-[#1b2e4b]">
                                    <FiEye/>
                                </div>
                            </div>
                        </div>
                        <div>
                            <label className="cursor-pointer">
                                <input type="checkbox" className="form-checkbox" value={checkPolicy} onChange={e =>setCheckPolicy(true)}/>
                                <span className="text-white-dark ml-3">
                                    J'accepte les {' '}
                                    <button type="button" className="text-lime-500 hover:underline">
                                        conditions générales d'utilisation.
                                    </button>
                                </span>
                            </label>
                        </div>
                        <button type="submit" disabled={processing} className="btn bg-lime-500 text-white w-full">
                            {/* loader if processing */}
                            {processing ?? <div className="loader ease-linear rounded-full border-2 border-t-2 border-gray-200 h-4 w-4 mb-1"></div>}
                            CREER UN COMPTE
                        </button>
                    </form>
                    ...
                    <p className="text-center">
                        ...
                        <Link href={route('login')} className="font-bold text-lime-500 hover:underline ltr:ml-1 rtl:mr-1 ml-3">
                            Connectez-vous
                        </Link>
                    </p>
                </div>
