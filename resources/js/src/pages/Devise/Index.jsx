...
export default function Index({ devises, success}) {
//DATA TABBLE########################################
...
const [initialRecords, setInitialRecords] = useState(sortBy(devises, 'id'));
...
const cols = [
  ...
  { accessor: 'type_devise', title: 'Type' },
...
useEffect(() => {
  setInitialRecords(() => {
    return devises.filter((item) => {
      return (
        item.id.toString().includes(search.toLowerCase()) ||
        item.type_devise.toLowerCase().includes(search.toLowerCase()) ||
...
//FIN DATA TABLE
  ...
  useEffect(()=>{
   ...
   const handleClick=(id)=>{
    Inertia.get(route('dashboard.liste-devise.edit',id));
  }
  ...
  const handelState=(modalId)=>{
    Inertia.get(route('dashboard.liste-devise.show',modalId));
  }
  const boutons = [
    ...
    <DefaultLayout>
      <div className="panel datatables">
        <div className="flex justify-between mb-5">
          <h3 className="text-lg">Liste des devises</h3>
        </div>
        
        <div className="flex md:items-center md:flex-row flex-col mb-5 gap-5">
        <Link href={route('dashboard.liste-devise.create')} className="btn btn-success">Création</Link>
        ...
        <DataTable
          ...
          columns={[
            ...
            {
              accessor: 'type_devise',
              title: 'Type',
              sortable: true,
              hidden: hideCols.includes('type_devise'),
              render: (row) => (
                toPascalCase(row.type_devise)
              )
            },
            {
              accessor: 'status',
