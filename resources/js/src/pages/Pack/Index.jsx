...
export default function Index({ pack,success }) {
 //DATA TABBLE########################################
 ...
 const [initialRecords, setInitialRecords] = useState(sortBy(pack, 'id'));
 ...
 const cols = [
   ...
   { accessor: 'name_pack', title: 'Pack' },
   { accessor: 'service_pack', title: 'Service' },
 ...
 useEffect(() => {
   setInitialRecords(() => {
     return pack.filter((item) => {
       return (
         item.id.toString().includes(search.toLowerCase()) ||
         item.name_pack.toLowerCase().includes(search.toLowerCase()) ||
         item.service_pack.toLowerCase().includes(search.toLowerCase()) ||
 ...
 //FIN DATA TABLE
  ...
  useEffect(()=>{
   ...
   const handleClick=(id)=>{
    Inertia.get(route('dashboard.list-pack.edit',id));
  }
  ...
  const handelState=(modalId)=>{
    Inertia.get(route('dashboard.list-pack.show',modalId));
  }
  ...
  const handelCibleValue= (service_pack)=>{
      const data=JSON.parse(service_pack)
      const valueData=data.map((item)=>
        item.value
      )
      return valueData
  }
  handelCibleValue(pack[0].service_pack)
    ...
    <DefaultLayout>
      <div className="panel datatables">
        <div className="flex justify-between mb-5">
          <h3 className="text-xl font-bold">Liste des packs</h3>
          <Link href={route('dashboard.list-pack.create')} className="btn btn-success">Créer un  pack</Link>
        </div>
        ...
        <DataTable
          ...
          columns={[
            { accessor: 'id', title: 'Id', sortable: true, hidden: hideCols.includes('id') },
            {
              ...
              title: 'Pack',
              sortable: true,
              hidden: hideCols.includes('name_pack'),
              render: (row) => (
                toPascalCase(row.name_pack)
              )
            },
            {
              accessor: 'service_pack',
              title: 'Services',
              sortable: true,
              hidden: hideCols.includes('service_pack'),
              render: (row) => (
                handelCibleValue(row.service_pack).join(' , ')
              )
            },
            {
              accessor: 'price',
