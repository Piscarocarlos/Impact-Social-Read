...
export default function Index({ operateurs, success }) {
  ...
  const [initialRecords, setInitialRecords] = useState(sortBy(operateurs, 'id'));
  ...
  const cols = [
    ...
    { accessor: 'type_operator', title: 'Type' },
  ...
  const handleClick = (id) => {
    Inertia.get(route('dashboard.type-operateur-service.edit', id));
  }
  const handelState = (modalId) => {
    Inertia.get(route('dashboard.type-operateur-service.show', modalId));
  }
    ...
    <DefaultLayout>
      <div className="panel datatables">
        <div className="flex justify-between mb-5">
          <h3 className="text-lg">Type d'opérateur de service</h3>
        </div>

        <div className="flex md:items-center md:flex-row flex-col mb-5 gap-5">
          <Link href={route('dashboard.type-operateur-service.create')} className="btn btn-success">Création</Link>
