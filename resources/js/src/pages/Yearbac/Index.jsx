...
export default function Index({ year }) {
  ...
  const [records, setRecords] = useState(year.slice(0, PAGE_SIZE));
  useEffect(() => {
    const from = (page - 1) * PAGE_SIZE;
    const to = from + PAGE_SIZE;
    setRecords(year.slice(from, to));
  }, [page]);

     //UPDATE STATUS OF YEARBAC
     const handleClick=(id)=>{
      Inertia.get(route('dashboard.annees-bac.edit',{id}));
    }
    ...
    <DefaultLayout>
      <div className="panel">
        <div className="flex justify-between">
          <h3 className="text-xl font-bold">Liste des années du baccalauréat</h3>
        </div>
        <DataTable
          columns={[
            { accessor: 'year_bac', title: "Années du baccalauréat" }, 
            ...
            { accessor: '', title: "Actions sur l'année",render: (row) => (
