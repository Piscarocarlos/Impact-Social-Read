...
export default function Index({ civilities, success}) {
//DATA TABBLE########################################
...
const [initialRecords, setInitialRecords] = useState(sortBy(civilities, 'id'));
...
const cols = [
  ...
  { accessor: 'type_civility', title: 'Type' },
...
useEffect(() => {
  setInitialRecords(() => {
    return civilities.filter((item) => {
      return (
        item.id.toString().includes(search.toLowerCase()) ||
        item.type_civility.toLowerCase().includes(search.toLowerCase()) ||
...
//FIN DATA TABLE
  ...
  const [modalId, setModalId] = useState("");
   ...
   const handleClick=(id)=>{
    Inertia.get(route('dashboard.liste-civilite.edit',id));
  }
  ...
  const handelState=(modalId)=>{
    Inertia.get(route('dashboard.liste-civilite.show',modalId));
  }
  const boutons = [
    ...
    <DefaultLayout>
      <div className="panel datatables">
        <div className="flex justify-between mb-5">
          <h3 className="text-lg">Liste des civilités</h3>
        </div>

        <div className="flex md:items-center md:flex-row flex-col mb-5 gap-5">
        <Link href={route('dashboard.liste-civilite.create')} className="btn btn-success">Création</Link>
        ...
        <DataTable
          ...
          columns={[
            ...
            {
              accessor: 'type_civility',
              title: 'Type',
              sortable: true,
              hidden: hideCols.includes('type_civility'),
              render: (row) => (
                toPascalCase(row.type_civility)
              )
            },
            {
              accessor: 'status',
