...
export default function Index({ beneficiares, success }) {
  ...
  const [initialRecords, setInitialRecords] = useState(sortBy(beneficiares, 'id'));
  ...
  const cols = [
    ...
    { accessor: 'type_beneficiary', title: 'Bénéficiare' },
  ...
  useEffect(() => {
    setInitialRecords(() => {
      return beneficiares.filter((item) => {
        return (
          item.id.toString().includes(search.toLowerCase()) ||
          item.type_beneficiary.toLowerCase().includes(search.toLowerCase()) ||
  ...
  useEffect(()=>{
   ...
   const handleClick=(id)=>{
    Inertia.get(route('dashboard.type-beneficiaire.edit',id));
  }
  const handelState=(modalId)=>{
    Inertia.get(route('dashboard.type-beneficiaire.show',modalId));
  }
    ...
    <DefaultLayout>
      <div className="panel datatables">
        <div className="flex justify-between mb-5">
          <h3 className="text-lg">Type de bénéficiare</h3>
        </div>
        <div className="flex md:items-center md:flex-row flex-col mb-5 gap-5">
        <Link href={route('dashboard.type-beneficiaire.create')} className="btn btn-success">Création</Link>
        ...
        <DataTable
          ...
          columns={[
            ...
            {
              accessor: 'type_beneficiary',
              title: 'Bénéficiare',
              sortable: true,
              hidden: hideCols.includes('type_beneficiary'),
              render: (row) => (
                toPascalCase(row.type_beneficiary)
              )
            },
            {
              accessor: 'status',
