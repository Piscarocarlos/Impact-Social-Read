...
export default function Index({ services, success}) {
 ...
 const [initialRecords, setInitialRecords] = useState(sortBy(services, 'id'));
 ...
 const [hideCols, setHideCols] = useState(['created_at','description',"operator"]);
 ...
 const cols = [
   ...
   { accessor: 'name_service', title: 'Service' },
   { accessor: 'description', title: 'Description' },
   { accessor: 'cible', title: 'Cible' },
   { accessor: 'cout_unitaire', title: 'Coût unitaire' },
   { accessor: 'operator_service', title: 'Opérateur' },
   ...
   setInitialRecords(() => {
     return services.filter((item) => {
       return (
         ...
         item.name_service.toLowerCase().includes(search.toLowerCase()) ||
         item.operator_service.toLowerCase().includes(search.toLowerCase()) ||
 ...
 //FIN DATA TABLE
  ...
  useEffect(()=>{
   ...
   const handleClick=(id)=>{
    Inertia.get(route('dashboard.service.edit',id));
  }
  ...
  const handelState=(modalId)=>{
    Inertia.get(route('dashboard.service.show',modalId));
  }
  const boutons = [
  ...
  handelCibleValue(services[0].cible)
    ...
    <DefaultLayout>
      <div className="panel datatables">
        <div className="flex justify-between mb-5">
          <h3 className="text-lg">Liste des services</h3>
        </div>
        <div className="flex md:items-center md:flex-row flex-col mb-5 gap-5">
            <Link href={route('dashboard.service.create')} className="btn btn-success">Création</Link>
        ...
        <DataTable
          ...
          columns={[
            ...
            {
              accessor: 'name_service',
              title: 'Service',
              sortable: true,
              hidden: hideCols.includes('name_service'),
              render: (row) => (
                toPascalCase(row.name_service)
              )
            },
            {
            ...
            {
              accessor: 'cout_unitaire',
              title: 'Coût unitaire',
              sortable: true,
              hidden: hideCols.includes('cout_unitaire'),
              render: (row) => (
                row.cout_unitaire
              )
            },
            {
              accessor: 'operator_service',
              title: 'Opérateur',
              sortable: true,
              hidden: hideCols.includes('operator_service'),
              render: (row) => (
                toPascalCase(row.operator_service)
              )
            },
            {
              accessor: 'status',
